...
public class QualifiedNameResolver {
  ...
  private static Optional<RexNode> resolveFieldWithoutAlias(
    ...
    // A quoted identifier that contains dots (`cluster.name`) is a single part, so the walk above has
    // nothing to descend and only matches a column literally called that. Vanilla flattens objects, so
    // there such a column really exists; where an object is a struct instead, the same name means the
    // path into it. Split and retry — only after the literal lookup failed, so a column genuinely named
   ...
   * reads. Anything else keeps the whole remainder as a single {@code ITEM} key, which is right for a
   * MAP — a vanilla {@code object} is typed {@code MAP<VARCHAR, ANY>} because vanilla stores objects
   * flattened, so {@code city.location.latitude} genuinely is one key there.
   *
   * <p>Joining the remainder unconditionally, as this did before, produced {@code ITEM($city,
   * 'location.latitude')} for a real nested struct. {@code SqlItemOperator} looks a dotted key up as
